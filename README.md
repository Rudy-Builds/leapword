...
- **Difficulty is `(par, detour)`, not par alone.** `detour` is how many moves *beyond* the letters that differ between START and END the dictionary forces on you. At detour 0, "make it look more like the target" solves the puzzle with no lookahead — which makes a detour-0 day not an easy puzzle so much as an absent one. The week still opened on one. It now runs **4·1 → 5·1 → 5·2 → 6·2 → 6·3**, raising exactly one axis per day so each day differs from the last in one legible way, and **detour-0 weekdays went from 20% of the schedule to 0.2%**. Saturday moved the same way, for the same reason.
